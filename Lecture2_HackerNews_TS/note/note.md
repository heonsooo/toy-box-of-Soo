...
## CH04_10 콜백없는 비동기코드 작성
- 웹 프론트엔드 개발자들은 비동기 코드로 오랫동안 고통받았다.
- JS 진영에서는 비동기코드를 어떻게하면 잘 처리할 수 있을까 언어적/엔지니어링 측면에서 오래 고민을 하였고 이에 promise라고 하는 메커니즘이 나왔다.
- Promise 메커니즘을 처음보면 콜백보다 좋은걸까? 라는 의심이 들기도 한다.
  -> 콜백보다 promise가 더 복잡한거 같은데 ?
  -> 단순한 콜백처리는 콜백이 더 편한 거 같은데 ?
- 그렇지만 promise 안착 이후 async, await가 나왔고 이를 이용해 비동기 함수를 이용해서 내부적인 메커니즘은 콜백처럼 비동기적으로 작동하지만 코드상으로는 완전한 **동기코드처럼 보이게 작성할 수 있는 문법체계이다.**
- 내부적으로 Promise 베이스로 비동기적으로 동작하지만, 코드적으로는 동기적으로 작성할 수 있는 것이 async await의 장점이다.
